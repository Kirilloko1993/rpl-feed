...
Источник: https://www.championat.com/football/_russiapl.html
Структура: <a href=".../match/<id>/"><span class="_win|_no|_live"
           title="DD.MM.YYYY. N-й тур. Home - Away (H : A)"></span></a>
Календарь отдаётся серверно — нужен только stdlib (urllib + re).
...
def parse(html: str) -> list[dict]:
    ...
    for m in SPAN.finditer(html):
        _href, mid, cls, title = m.group(1), m.group(2), m.group(3), m.group(4)
...
def main() -> int:
    ...
    print(f"OK: {len(fixtures)} матчей (сыграно {played}) -> {out_path}")
    return 0
